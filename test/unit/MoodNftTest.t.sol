// SPDX-License-Identifier: MIT

pragma solidity 0.8.20;

import {Test, console} from "forge-std/Test.sol";
import {MoodNft} from "../../src/MoodNft.sol";

contract MoodNftTest is Test {
    MoodNft public moodNft;
    string public constant HAPPY_SVG_URI =
        "data:image/svg+xml;base64,";

    string public constant SAD_SVG_URI =
        "data:image/svg+xml;base64,";

    address MINTER = makeAddr("mint");

    function setUp() public {
        moodNft = new MoodNft(SAD_SVG_URI, HAPPY_SVG_URI);
    }

    function testViewTokenURI() public {
        vm.prank(MINTER); // Without this line, it means it is the contract that is calling the mint function
        moodNft.mintNft();
        console.log(moodNft.tokenURI(0));
    }

    function testMintNft() public {
        vm.prank(MINTER);
        moodNft.mintNft();
        assertEq(moodNft.balanceOf(MINTER), 1);
    }

    function testToEnsureDefaultMoodIsHappy() public {
        vm.prank(MINTER);
        moodNft.mintNft();
        console.log(uint256(moodNft.getTokenIdToMood(0)));
        assert(moodNft.getTokenIdToMood(0) == MoodNft.Mood.HAPPY);
    }

    function testTokenCounterIsIntializeAsZero() public view {
        assertEq(moodNft.getTokenCounter(), 0);
    }

    function testToEnsureTokenCounterIncrement() public {
        vm.prank(MINTER);
        moodNft.mintNft();
        uint256 tokenCounter = moodNft.getTokenCounter();
        assertEq(tokenCounter, 1);
    }

    function testToEnsureOnlyOwnerOrApprovedCanCallFlipMood() public {
        vm.prank(MINTER);
        moodNft.mintNft();
        vm.expectRevert(MoodNft.MoodNft__CantFlipMoodIfNotOwner.selector);
        moodNft.flipMood(0);
    }

    function testFlipMood() public {
        vm.prank(MINTER); //vm.prank(address) sets the msg.sender (caller) for the next transaction only. After the next transaction, the caller reverts to the default behavior (the test contract's address).
        moodNft.mintNft();
        vm.prank(MINTER);
        moodNft.approve(MINTER, 0);
        vm.prank(MINTER);
        moodNft.flipMood(0);

        assert(moodNft.getTokenIdToMood(0) == MoodNft.Mood.SAD);
    }
}
